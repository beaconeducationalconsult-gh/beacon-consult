...
export class CreativeArtsDesignModule implements SubjectModule {
  ...
  async loadIndicators(grade: string): Promise<Indicator[]> {
    ...
    return raw
      .filter((i: any) => i.subjectId === 'creative-arts-design')
      .map((i: any) => ({
        code: i.code,
        strand: i.strandName,
  ...
  generateSchemeOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Scheme of Work — Creative Arts & Design ${grade} Term ${term}`,
      sections: [
        {
      ...
      meta: { subject: 'creative-arts-design', grade, term },
    }
  }
  ...
  generateRecordOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Record of Work — Creative Arts & Design ${grade} Term ${term}`,
      sections: [
        {
  ...
  async generateQuestionsDoc(indicator: Indicator, grade: string, term: number): Promise<MaterialDoc> {
    return {
      ...
      meta: { subject: 'creative-arts-design', grade, term, indicatorCode: indicator.code },
    }
  }

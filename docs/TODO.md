...
| P1-6 | ~~**Clean the L2 `ind_desc` records with a repeated trailing sentence**~~ — **mostly done 2026-09-18**: `scripts/fix_ind_desc_exemplars.py` cut the print's exemplar tail off **242 records in 8 databases** (239 dangling exemplar intros — the creative-arts `Learners are to` family — and 3 repeats) and, because the lesson template is a copy, out of **980 lesson slots in 8 lesson files**: `ind_desc` 980, `perf_indicator` 980, `starter` 805, `main` 805. Every cut is verified against the row the print sets and recorded in `data/audit/ind_desc_exemplars.json`. What is left needs a judgement call rather than a cut: **151 records** whose tail is real exemplar content the indicator does not carry (career-technology's numbered steps, history's `Enquiry route:` blocks) and **32 records the print does not back** because the **core-competencies column bled into `ind_desc`** — those need the column-bleed pass, and ten creative-arts B5/B6 indicators turn out not to match their print at all (listed in the artefact) | Visible artefact in every generated document | M |

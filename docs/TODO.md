...
| P2-3 | **Reconcile the Python spec with the TS runtime** (`ncos/kernel/interface.py` is sync and manifest-driven; `legacy/kernel-app/src/kernel/` is async and class-driven), and add an `interface_version` check on the JS side | Principle #3 in `README.md` is currently untrue, and nothing detects further drift | M |

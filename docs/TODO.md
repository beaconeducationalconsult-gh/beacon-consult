...
| P3-2 | **Offline packs** — per subject/class downloads for schools without reliable connectivity | The prerequisite landed with P2-3: the served unit *is* the subject-grade (`schedules/<grade>-<subject>.json`), and the service worker caches each file on first use. What is still missing is an explicit "download this subject for offline" affordance that warms the cache and reports progress |

...
| P1-11 | **74 served records are label-form placeholders** — every `owop_B4`/`B5`/`B6` record (25/25/24) reads `"Our World Our People Content Standard B4.1.1.1"` / `"…Learning Indicator B4.1.1.1.1"` where the print sets the standard's and indicator's own words; 228 more sit inert in `data/reference/` | Teachers read the code restated instead of the curriculum, and generated documents inherit it. This is a **filling** job: `owop_B4-B6.pdf` carries the text (spot-checked pp. 18/40/61), and `scripts/drop_front_matter_records.py --details` prints all 302 label-form records with the reason each is kept | M |
...
that restate their code are P1-11) and the french

...
| The setup notice instead of the portal | The **deploy** was built without the six `VITE_FIREBASE_*` values (a local build can be fine) | `node scripts/verify_deploy.mjs -Url <deploy>`, or `GET /build-info.json` — it names the missing values. The notice itself prints them too when the reader is on a real domain (it branches on dev vs deployment). Set them on Vercel for Production **and** Preview, then **redeploy** — a variable change does not rebuild anything on its own |

...
export class CreativeArtsModule implements SubjectModule {
  id = 'creative-arts'
  displayName = 'Creative Arts'
  grades = ['B2', 'B3', 'B4', 'B5', 'B6']
  ...
  async loadIndicators(grade: string): Promise<Indicator[]> {
    ...
    return raw
      .filter((i: any) => i.subjectId === 'creative-arts')
      .map((i: any) => ({
        code: i.code,
        strand: i.strandName,
  ...
  generateSchemeOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Scheme of Work — Creative Arts ${grade} Term ${term}`,
      sections: [
        {
      ...
      meta: { subject: 'creative-arts', grade, term },
    }
  }
  ...
  generateRecordOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Record of Work — Creative Arts ${grade} Term ${term}`,
      sections: [
        {
  ...
  async generateQuestionsDoc(indicator: Indicator, grade: string, term: number): Promise<MaterialDoc> {
    return {
      ...
      meta: { subject: 'creative-arts', grade, term, indicatorCode: indicator.code },
    }
  }

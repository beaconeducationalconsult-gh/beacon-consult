...
export class SocialStudiesModule implements SubjectModule {
  id = 'social-studies'
  ...
  async loadIndicators(grade: string): Promise<Indicator[]> {
    ...
    return raw
      .filter((i: any) => i.subjectId === 'social-studies')
      .map((i: any) => ({
        code: i.code,
        strand: i.strandName,
  ...
  generateSchemeOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Scheme of Work — Social Studies ${grade} Term ${term}`,
      sections: [
        {
      ...
      meta: { subject: 'social-studies', grade, term },
    }
  }
  ...
  generateRecordOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Record of Work — Social Studies ${grade} Term ${term}`,
      sections: [
        {
  ...
  async generateQuestionsDoc(indicator: Indicator, grade: string, term: number): Promise<MaterialDoc> {
    return {
      ...
      meta: { subject: 'social-studies', grade, term, indicatorCode: indicator.code },
    }
  }

...
export class ComputingModule implements SubjectModule {
  id = 'computing'
  ...
  async loadIndicators(grade: string): Promise<Indicator[]> {
    ...
    return raw
      .filter((i: any) => i.subjectId === 'computing')
      .map((i: any) => ({
        code: i.code,
        strand: i.strandName,
  ...
  generateSchemeOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Scheme of Work — Computing ${grade} Term ${term}`,
      sections: [
        {
      ...
      meta: { subject: 'computing', grade, term },
    }
  }
  ...
  generateRecordOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Record of Work — Computing ${grade} Term ${term}`,
      sections: [
        {
  ...
  async generateQuestionsDoc(indicator: Indicator, grade: string, term: number): Promise<MaterialDoc> {
    return {
      ...
      meta: { subject: 'computing', grade, term, indicatorCode: indicator.code },
    }
  }

...
export class FrenchModule implements SubjectModule {
  id = 'french'
  displayName = 'French'
  grades = ['B7', 'B8', 'B9']
  ...
  async loadIndicators(grade: string): Promise<Indicator[]> {
    ...
    return raw
      .filter((i: any) => i.subjectId === 'french')
      .map((i: any) => ({
        code: i.code,
        strand: i.strandName,
  ...
  generateSchemeOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Scheme of Work — French ${grade} Term ${term}`,
      sections: [
        {
      ...
      meta: { subject: 'french', grade, term },
    }
  }
  ...
  generateRecordOfWork(grade: string, term: number, options?: Record<string, unknown>): MaterialDoc {
    ...
    return {
      title: `Record of Work — French ${grade} Term ${term}`,
      sections: [
        {
  ...
  async generateQuestionsDoc(indicator: Indicator, grade: string, term: number): Promise<MaterialDoc> {
    return {
      ...
      meta: { subject: 'french', grade, term, indicatorCode: indicator.code },
    }
  }
